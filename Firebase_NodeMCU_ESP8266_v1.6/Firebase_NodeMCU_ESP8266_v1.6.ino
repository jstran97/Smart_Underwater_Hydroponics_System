...
*/
#define WIFI_SSID "NETGEAR02"
#define WIFI_PASSWORD "pasteldiamond934"
#define FIREBASE_HOST "lagoon-9de60.firebaseio.com"
#define FIREBASE_AUTH "aWjHUYVg9d7ZJEqKVW8tgaRHf4WlRt90FxFgze3H"

int count; // Indicates how many values were updated in database
char pump_str[1];
int numBytes;
...
bool isDoneOnce; // Used to limit number of times sensor data is transmitted
...
void setup() {
  ...
  isDoneOnce = false; // Allows for one sensor data transmission to Firebase 
                      // every 15 minutes
  numBytes = 0; // Initialize number of bytes to 0 
  count = 1; // for 1st value to be pushed and stored in database
}
...
void loop() {
  ...
  if (((current_hour % 4) == 0) && (current_min <= 14)) {
    setPumpState(WATER_PUMP_ON);   
  ...
  // Obtain data from Mega 2560 board every 5 minutes (0, 5, 10, 15 minute mark, etc)
  if (current_min % 5 == 0 && isDoneOnce == false) {
...
//      clearIncomingBuffer(); // Empty incoming serial data buffer (from Mega2560)
      ...
      // As long as char pointer is not pointing to NULL (or '\0') and
      // 4 numbers have not been stored, then keep on looking for numbers in char array
      while (ch_ptr_rcv != NULL && float_index < 4) {
        delay(1);
        ch_ptr_rcv = strtok(NULL, ","); // Look for next start of token with comma (,)
                                        // and returns pointer to first char of 
                                        // segment before the comma (,) detected
